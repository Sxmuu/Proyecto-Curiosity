...
void imprimirElementos(list <sElemento> elementos){
    for(auto it=elementos.begin();it!=elementos.end();it++){
        ...
        cout<<"Tamanio: " <<it->tamanio << " "<<it->unidadMedida<<endl;
...
list<char*> entrada;
    ...
    if(strcmp(comando,"help")==0){
        ...
        if(i < 2){
        ...
        else if(Curiosity.getElementos().size() == 0){
            cerr << "Error: no hay elementos para crear el mapa\n";
            return;
        }
        else{
            advance(it, 1);
            ...
            if(indicedeconectividad<=0 || indicedeconectividad>1){
                cerr << "Error: el indice de conectividad no es valido\n";
                return;
            }
        ...
        else if(Curiosity.mapa->vacio()){
            cerr << "Error: no se ha creado el mapa\n";
            return;
        }
        else{
